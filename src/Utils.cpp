...
{
    if (str == L"fullsingle" || str == L"完整單次") return ScriptRunMode::FullSingle;
    else if (str == L"fullcontinuous" || str == L"完整連續")  return ScriptRunMode::FullContinuous;
    else if (str == L"fullswitch" || str == L"完整切換") return ScriptRunMode::FullSwitch;
...
#ifdef _LANG_ZH_TW_
        outError = L"括號 ')' 後面不應該有其他文字，如有需要可以將文字加在 # 之後;";
#else
